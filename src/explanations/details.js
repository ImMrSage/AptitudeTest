    ...
    export function buildDetailedExplanationHtml(q) {
      ...
      if (q.topic === 'numerical' || q.topic === 'quantitative') {
        if (q.variantKey === 'numerical-bar-profit') {
          ...
          const profitLines = names.map((name, i) => {
            ...
            return `<div class="formula-line">${name}: ${(unitMatches[i] || 0)} Ã— ${scale} = $${profit.toLocaleString()} ; ${(hours[i] || 0)} Ã— ${wage} = $${wageCost.toLocaleString()}</div>`;
          }).join('');
          return `
            <div><strong>Formula setup</strong></div>
            <div class="formula-block">
              <div class="formula-line">Profit = bar units Ã— bar value</div>
              <div class="formula-line">Wage = hours Ã— hourly wage</div>
              ${profitLines}
            </div>
        ...
        if (q.variantKey === 'numerical-pie-growth') {
          ...
          return `
            ...
            <div class="formula-block">
              <div class="formula-line">Base sales = ${total} Ã— ${regionPct}% = ${total} Ã— ${regionPct / 100} = ${base.toFixed(1)} million</div>
            </div>
            <div><strong>Step 2</strong></div>
            <div class="formula-block">
              <div class="formula-line">2021 sales = ${base.toFixed(1)} Ã— (1 + ${growth}/100)</div>
              <div class="formula-line">2021 sales = ${base.toFixed(1)} Ã— ${(1 + growth / 100).toFixed(2)} = ${result.toFixed(1)} million</div>
            </div>
          `;
        }
        ...
        if (q.variantKey === 'numerical-loan-fine') {
          ...
          return `
            <div class="formula-block">
              ...
              <div class="formula-line">Total loans = ${totalUnits} Ã— ${mult.toLocaleString()} = ${totalLoans.toLocaleString()}</div>
              <div class="formula-line">Late loans = ${totalLoans.toLocaleString()} Ã· ${ratio} = ${late.toLocaleString()}</div>
              <div class="formula-line">Fine income = ${late.toLocaleString()} Ã— $${fine} = $${Math.round(income).toLocaleString()}</div>
            </div>
          `;
        }
          ...
          if (rowMatch) {
            ...
            return `
              <div class="formula-block">
                ...
                <div class="formula-line">Good units = ${units} Ã— ${(100 - defect) / 100} = ${good}</div>
              </div>
            `;
          }
        }
        ...
        if (q.variantKey === 'numerical-discount-table') {
          ...
          if (priceMatch && discountMatch) {
            ...
            return `
              <div class="formula-block">
                <div class="formula-line">Original total = ${qty} Ã— $${price.toFixed(2)} = $${original.toFixed(2)}</div>
                <div class="formula-line">Discounted total = $${original.toFixed(2)} Ã— ${(1 - discount / 100).toFixed(2)} = $${total.toFixed(2)}</div>
              </div>
            `;
          }
        }
          ...
          if (q.variantKey === 'numerical-averages-mean') {
            return `
              <div class="formula-block">
                ...
                <div class="formula-line">Average = ${total} Ã· ${scores.length} = ${(total / scores.length).toFixed(0)}</div>
              </div>
            `;
          }
        ...
        if (q.variantKey === 'numerical-time-work') {
          ...
          return `
            <div class="formula-block">
              ...
              <div class="formula-line">Time = 1 Ã· ${rate.toFixed(3)} = ${(1 / rate).toFixed(1)} hours</div>
            </div>
          `;
        }
      ...
      if (q.topic === 'mechanical') {
        ...
        if (q.variantKey.includes('lever has the load close')) {
          return `
            <div class="formula-block">
              <div class="formula-line">Turning effect (moment) = force Ã— distance from pivot</div>
